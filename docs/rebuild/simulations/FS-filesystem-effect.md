...
*Role-play as Mentl, tracing what happens when `cache.ka` calls
`perform fs_write_file("synth/core.ka.kai", bytes)`. Today the
...
`cache.ka` (IC work) needs to:

1. Check whether `.inka/cache/infer.ka.kai` exists.
...
- runtime/io.ka primitives that pack arguments into the iov
...
## Layer 3 — runtime/io.ka primitives
...
In `pipeline.ka` (or a new `std/compiler/filesystem.ka` if it grows):
...
- `inka check <path>` / `inka build <path>` can read `.ka` source
  files from disk (today main.ka reads stdin only — requires FS
...
**runtime primitives** — 4 impl functions in runtime/io.ka using
...
- IC.1 (cache.ka) is the immediate consumer; FS lands just before IC.1.
...
- ~150 lines across 3 files:
  - `types.ka` — Filesystem effect decl (~20 lines)
  - `runtime/io.ka` — 4 impl fns + scratch layout (~100 lines)
  - `pipeline.ka` — wasi_filesystem handler + install (~30 lines)
- `backends/wasm.ka` emit header gains 4 import lines.
