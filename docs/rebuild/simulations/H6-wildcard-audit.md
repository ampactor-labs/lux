...
|------------------|----------------------|------------------------------------------------|
| `Ty`             | types.ka             | Every type-directed decision reads its shape   |
| `NodeBody`       | types.ka             | AST walk discriminator; every pass matches     |
| `Stmt`           | types.ka             | Statement-level variants (FnStmt, LetStmt, …)  |
| `Expr`           | types.ka             | Expression-level variants (PipeExpr, VarRef, …)|
| `Pat`            | types.ka             | Pattern-level variants                          |
| `LowExpr`        | lower.ka             | LIR walk; every emit and analysis matches      |
| `EffRow`         | types.ka             | Effect algebra's shape discriminator           |
| `Reason`         | types.ka             | Why Engine rendering, graph reason-edge walk   |
| `PipeKind`       | types.ka             | Verb dispatch in infer/lower                   |
| `Annotation`     | mentl.ka             | Oracle candidate discriminator                 |
| `Resolution`     | types.ka             | Scope resolution discriminator                 |
| `NodeKind`       | types.ka             | Graph node kind (NFree, NBound, …)             |
...
> added to types.ka. It carries its own effect row because thunks
...
**Today, infer.ka:1145 (chase_deep):**
...
- `infer.ka:1157` (another chase variant)
- `infer.ka:1188` (free_in_ty)
- `infer.ka:1248` (subst_ty)
- `graph.ka:338` (occurs_in — most dangerous; missed occurs → infinite types)
...
Same pattern applies to:
- `infer.ka:1078` (infer_pat — new Pat variants silently ignored)
- `own.ka:355` (count_uses — new NExpr silently has zero uses of any name)
- `own.ka:281` (walk_return_positions — new NExpr silently doesn't return refs)
- `lower.ka:628` (cfv_expr — new NExpr silently captures nothing)
- `lower.ka:676` (lower_expr_body fallback — new Expr silently lowers to LConst(0))

---

## Simulation 4 — the LIR-backend trap

**Today, backends/wasm.ka:342 (and 619, 692):**
...
|------------------------------|---------------|-------------|---------------------------|----------|
| mentl.ka:424                 | `Ty`          | `_ => EfPure` | New `Ty` → row silently Pure | HIGH |
| pipeline.ka:295              | `Ty`          | `_ => "Pure"` | New `Ty` → diagnostic says Pure | HIGH |
| infer.ka:1135                | `NodeKind`    | `_ => Forall([], TVar(handle))` | Fabricated scheme | HIGH (CLAUDE.md-named) |
| infer.ka:1145                | `Ty`          | `_ => ty` | New `Ty` with sub-types → stale handles | HIGH |
| infer.ka:1157                | `Ty`          | `_ => ty` | Same as above | HIGH |
| infer.ka:1188                | `Ty`          | `_ => []` | New `Ty` with TVars → missing free-var inference | HIGH |
| infer.ka:1248                | `Ty`          | `_ => ty` | New `Ty` subst-blind | HIGH |
| graph.ka:338                 | `Ty`          | `_ => false` | occurs_in misses new shape → infinite types | CRITICAL |
| infer.ka:518                 | `NodeBody`    | `_ => ()` | New `Expr` never inferred | HIGH |
| infer.ka:1078                | `Pat`         | `_ => ()` | New `Pat` silently unbinds | HIGH |
| own.ka:281                   | `NodeBody`    | `_ => ()` | New `Expr` can't escape-check | HIGH |
| own.ka:355                   | `NodeBody`    | `_ => 0`  | New `Expr` has zero own-uses | HIGH |
| lower.ka:362                 | `NodeBody`    | `_ => LConst(handle, LInt(0))` | New body-kind → integer zero emitted | HIGH |
| lower.ka:378                 | `Pat`         | `_ => lo` | New `Pat` doesn't bind name | MEDIUM |
| lower.ka:446                 | `Pat`         | `_ => ()` | New `Pat` unbound in cfv | MEDIUM |
| lower.ka:628                 | `Expr`        | `_ => []` | New `Expr` captures nothing | HIGH |
| lower.ka:635                 | `Stmt`        | `_ => []` | New `Stmt` captures nothing | HIGH |
| lower.ka:676                 | `Expr`        | `_ => { LConst(h, LInt(0)) }` | New `Expr` lowers to 0 | HIGH |
| lower.ka:718                 | `Pat`         | `_ => []` | New `Pat` binds nothing | MEDIUM |
| lower.ka:806                 | `NodeBody`    | `_ => acc` | New top-level stmt kind not indexed | MEDIUM |
| backends/wasm.ka:342         | `LowExpr`     | `_ => acc` | New `LowExpr` invisible to fn table | HIGH |
| backends/wasm.ka:359         | `LowExpr`     | `_ => ""`  | New top-level kind has no name | MEDIUM |
| backends/wasm.ka:512         | `LowExpr`     | `_ => 0` | New `LowExpr` doesn't count arity | HIGH |
| backends/wasm.ka:619         | `LowExpr`     | `_ => ()` | New `LowExpr` not emitted to fn table | HIGH |
| backends/wasm.ka:692         | `LowExpr`     | `_ => ()` | New `LowExpr` doesn't declare locals | HIGH |
| infer.ka:883                 | `Ty`          | `_ => false` | expect_same misses new primitive | MEDIUM |
| infer.ka:944                 | `Ty`          | `_ => arity_mismatch(...)` | Non-tuple type → arity error | BOUNDARY |

### SAFE (keep; add one-line comment naming contract)

| File:line                    | Match subject | Arm           | Contract                                           |
|------------------------------|---------------|---------------|----------------------------------------------------|
| graph.ka:235, 242, 244, 255  | `NodeKind`/`Ty`| `_ => chased` / `_ => GNode(kind, reason)` | Identity preserve — chase terminates on non-indirection |
| graph.ka:265                 | `EffRow`      | `_ => row`    | Merge has no slot for this row shape — structure preserved |
| effects.ka:131               | `EffRow`      | `_ => EfNeg(row)` | Default negation for non-algebraic rows |
| effects.ka:155,157,174,176,178 | `EffRow`    | `_ => EfInter(a, b)` | Deferred intersection — algebra can't simplify |
| effects.ka:269,275,282,292,294| `EffRow`     | `_ => false`  | Subsumption refuses to prove what isn't provable |
| effects.ka:332,346,368,370   | `EffRow`      | `_ => unify_row_canonical(normalize_row(...), ...)` | Re-enter via canonicalization |
| infer.ka:1425                | `Ty`          | `_ => []`     | Non-function types have no effect row names |
| infer.ka:1434                | `EffRow`      | `_ => []`     | Non-named rows have no names |
| infer.ka:1440                | `NodeKind`    | `_ => EfPure` | Unresolved row handle defaults to Pure for read |
| infer.ka:572, 639            | `GNode`       | `_ => ()`     | Non-NRowBound chase results: row isn't resolved yet |
| infer.ka:597                 | `NodeKind`    | `_ => { … }`  | Fallback branch for non-TFun op types — reports diagnostic |
| infer.ka:865, 883            | `Ty`          | `_ => type_mismatch(...)` | Explicit mismatch — the DOCUMENTED fallback |
| mentl.ka:142, 144            | `NodeKind`    | `_ => None`   | gradient_next returns None when handle isn't a fn |
| mentl.ka:209                 | `Ty`          | `_ => ()`     | narrow_row: only narrow TFun rows; no-op for others |
| mentl.ka:347                 | `Reason`      | `_ => reason` | why_expand identity-preserves leaves |
| mentl.ka:376                 | `NodeKind`    | `_ => resume(false)` | verify_after_apply: only NBound proves; else false |
| mentl.ka:418                 | (proven list) | `_ => None`   | No proven annotations → None |
| mentl.ka:465                 | String        | `_ => "Unknown error code: …"` | Explicit unknown — the DOCUMENTED fallback |
| own.ka:251                   | `Own`         | `_ => collect_ref_params_loop(...)` | Non-Ref ownership: skip in ref-param collection |
| own.ka:312                   | `Own`         | `_ => own`    | Preserve Own/Ref markers; only Inferred is reclassified |
| pipeline.ka:250, 284         | `NodeBody`    | `_ => []`     | Query iterator: only top-level FnStmts contribute |
| pipeline.ka:344, 348, 354, 356 | String      | `_ => QUnknown(q)` | Explicit unknown query variant |
| lower.ka:144                 | `Ty`          | `_ => true`   | monomorphic_at: non-function types are trivially monomorphic |
| lower.ka:319                 | `LowExpr`     | `_ => [LCall(...)]` | Single-branch `<\|` — treat as one LCall |
| lower.ka:340                 | `Ty`          | `_ => 0`      | Field offset lookup: non-record types → 0 |
| lower.ka:569                 | `NodeBody`    | `_ => []`     | cfv_node: NPat/NHole contribute no free vars |
| backends/wasm.ka:1071, 1083  | String        | `_ => { … }`  | Unknown binop/unaryop — diagnostic comment |
...
5. **Runtime files (std/runtime/)** are out of scope for H6 unless they have load-bearing ADT matches. Quick sweep confirms they don't — lists.ka and strings.ka operate on scalar structures.
