...
`teach_synthesize(hole_handle, context)` (`mentl.ka:476-487`) asks
...
All edits land inside `std/compiler/mentl.ka`. No other file is
...
- `graph_push_checkpoint` / `graph_rollback` / `graph_bind` —
  LIVE (graph.ka).
- `row_subsumes` — LIVE (effects.ka).
- `Candidate(Node, Reason)` ADT — LIVE (mentl.ka).
- `Synth` effect + `synth_enumerative` handler — LIVE (mentl.ka:
  420-454).
- `verify_after_apply` — LIVE (mentl.ka:288-297).
