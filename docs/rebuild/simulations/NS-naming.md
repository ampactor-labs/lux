...
- Item 10 — `.ka` → `.nx` extension migration
...
**Substrate requirement** (verified against current parser + infer state). Dot-access on imported modules requires inference to accept `FieldExpr(VarRef(module_name), field_name)`. The parser already constructs this shape (`parser.ka:704`); inference currently constrains the left-hand side of `FieldExpr` to a record (record-open unification at `infer.ka:602`).
...
**Current form.** `src/compiler/lexer.ka` and `src/compiler/parser.ka`. Agent-named ("the thing that acts").
...
**Current state.** Some modules open with rich purpose statements (e.g., `src/types.ka`'s "THE vocabulary of Inka"). Others open with one-liners. No canonical top-of-module documentation form.
...
```
mv src/compiler/lexer.ka → src/lex.nx
mv src/compiler/parser.ka → src/parse.nx
```
...
**Before** (current `src/compiler/cache.ka`, approximate):
```
// cache.ka — IC cache for module envs
```
...
3. Simplification execution (item 11) runs across the whole `src/` + `lib/` tree in ONE focused series of commits:
   ...
   - Commit E: file renames (`lexer.ka` → `lex.nx`, `parser.ka` → `parse.nx`) + every import updated. (Fold into restructure item 17' for extension migration.)
