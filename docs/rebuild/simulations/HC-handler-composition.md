...
current queue. Seed instance: prelude.ka's Iterate family (map /
...
## 1. The seed instance — prelude.ka's Iterate family
...
**No types.ka change.** Effect signatures stay; just handler body
rewrites.
...
Net: -40 lines in prelude.ka. Cleaner.
...
```
bash tools/drift-audit.sh std/prelude.ka
```
...
2. **Code refactor** lands as commit 11.C.3:
   - std/prelude.ka rewrites (map_h, filter_h, take_h, skip_h,
