...
CHUNKS=(
  ...
  "bootstrap/src/emit/emit_const.wat"     # Tier 6 (uses $lexpr_handle + $lexpr_lconst_value from lower/lexpr + $lookup_ty from lower/lookup + $emit_is_terror_hole from emit/lookup + $emit_string_intern from emit/state + $ty_tag from infer/ty + $emit_byte/_str/_i32_const/_open/_close from emit_infra; Hβ-emit §2.1 + §3.5 + §5.1 + §7.1 + §11.3 — LConst tag-300 emit arm; 1 export — $emit_lconst (4-arm Ty dispatch — TError-hole→"(unreachable)" + TString→intern+i32.const + TUnit→i32.const 0 + TInt-and-fall-through→i32.const value per "heap has one story"); static data 1520 "unreachable" length-prefixed in [1518,1600) free zone post emit_data._start_fn pre emit_diag.ERROR_DEEP_CHASE; Lock #1 LMake* arms + $emit_lexpr partial dispatcher deferred to peer Hβ.emit.const-make-arms per Hβ.lower walk_call.wat precedent — LConst has no sub-LowExprs so lands cleanly without dispatcher; named follow-ups: const-make-arms, float-substrate)
