...
CHUNKS=(
  ...
  "bootstrap/src/lower/walk_compound.wat" # Tier 7 (uses lookup + lexpr + cross-layer infer/walk_expr.wat + cross-chunk lower/walk_call.wat $lower_expr; Hβ.lower §4.2 + §6.3 + §11 + §12.3 #9; 10 exports — $lower_unary_op (UnaryOpExpr→LUnaryOp) + $lower_lambda (LambdaExpr→LMakeClosure per Lock #1 caps+evs empty seed pending Hβ.lower.lambda-capture-substrate) + $lower_if (IfExpr→LIf single-element branches per Lock #10) + $lower_block (BlockExpr→LBlock final-only per Lock #2 pending Hβ.lower.blockexpr-stmts-substrate) + $lower_match (MatchExpr→LMatch arms-empty per Lock #3 pending Hβ.lower.match-arm-pattern-substrate) + $lower_make_list / $lower_make_tuple (LMakeList/LMakeTuple) + $lower_make_record + $lower_named_record (LMakeRecord — Lock #5 H2.3 nominal-collapse, Lock #6 fields via record_get offset 4) + $lower_field (FieldExpr→LFieldLoad offset-0 seed per Lock #4 pending Hβ.lower.field-offset-resolution); RETROFITS walk_call.wat $lower_expr with tag-87/89/90/91/92/96/97/98/99/100 arms; named follow-ups: lambda-capture-substrate, blockexpr-stmts-substrate, match-arm-pattern-substrate, field-offset-resolution, compound-mk-constructors, makerecord-promotion, lower-expr-dispatch-extension)
