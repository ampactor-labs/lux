...
(module
  ...
  ;;           "Forall([qid], TFun([qid], qid, EfPure))" — fresh-handle
  ...
  ;;               $generalize chases h_fn (NBound(TVar(h_q))), collects
  ;;               h_q via $free_in_ty, wraps as Forall([h_q], TVar(h_q)).
  ...
  ;;               Instantiation("inst", Fresh(old)) — tag 232. We assert
  ;;               that tag on the fresh node's reason field.
  ...
  (data (i32.const 3200) "\10\00\00\00body-not-tvar   ")
  (data (i32.const 3224) "\14\00\00\00body-wrong-handle   ")
  (data (i32.const 3248) "\18\00\00\00fresh-collides-with-old ")
  (data (i32.const 3280) "\10\00\00\00inst-not-tvar   ")
  (data (i32.const 3304) "\10\00\00\00scheme-mutated  ")
  (data (i32.const 3328) "\18\00\00\00fresh-reason-wrong-tag  ")

  ;; ─── _start ──
  (func $_start (export "_start")
    (local $failed i32)
    (local $h_q i32) (local $tv i32) (local $h_fn i32)
    (local $scheme i32) (local $qs i32) (local $body i32)
    (local $inst i32) (local $h_fresh i32) (local $g_fresh i32)
    (local.set $failed (i32.const 0))
    (call $graph_init)

    ;; Allocate free quantified handle
    (local.set $h_q (call $graph_fresh_ty (i32.const 0)))
    ;; Build TVar(h_q) and bind h_fn to it
    (local.set $tv (call $ty_make_tvar (local.get $h_q)))
    (local.set $h_fn (call $graph_fresh_ty (i32.const 0)))
    (call $graph_bind (local.get $h_fn) (local.get $tv) (i32.const 0))
    ...
    ;; Assert body is TVar (tag 104)
    (if (i32.ne (call $ty_tag (local.get $body)) (i32.const 104))
      (then
        (call $eprint_string (i32.const 3200))
        (call $eprint_string (i32.const 3104))
        (local.set $failed (i32.const 1))))

    ;; Assert body's TVar handle is h_q
    (if (i32.ne (call $ty_tvar_handle (local.get $body)) (local.get $h_q))
      (then
    ...
    ;; Assert inst is TVar (tag 104)
    (if (i32.ne (call $ty_tag (local.get $inst)) (i32.const 104))
      (then
        (call $eprint_string (i32.const 3280))
        (call $eprint_string (i32.const 3104))
        (local.set $failed (i32.const 1))))

    (local.set $h_fresh (call $ty_tvar_handle (local.get $inst)))
    ...
    ;; Scheme not mutated by instantiate (qs[0] still h_q)
    (if (i32.ne (call $list_index (local.get $qs) (i32.const 0)) (local.get $h_q))
      (then
